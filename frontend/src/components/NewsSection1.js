...
const CriminalActsSlider = () => {
  return (
    <div className="relative flex items-center justify-center p-4  shadow-md rounded-lg flex-col">
      <h2 className="text-black text-2xl font-bold mb-4 news-text">NEWS</h2>
      <Swiper
      ...
      >
        {criminalActs.map((act, index) => (
          <SwiperSlide key={index}>
            <div className="flex rounded-lg shadow-lg overflow-hidden bg-[#545677]">
              {/* Image Section */}
              <div className="flex-shrink-0 w-1/3 border-r border-gray-700 overflow-hidden">
                <img
                  src={act.image}
                  alt={`${act.headline} - ${act.detected_crimes[0].crime}`}
              ...
              {/* Text Section */}
              <div className="flex-1 p-6 text-[#011c27]">
